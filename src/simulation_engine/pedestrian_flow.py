...
class PedestrianFlowSimulation:
    ...
    def calculate_forces(self, positions, velocities, exit_positions, obstacles):
        ...
        desired_directions /= np.linalg.norm(desired_directions, axis=1)[:, np.newaxis]
        ...
        for i in range(self.num_pedestrians):
            ...
            dist[i] = np.inf
            repulsive_force = np.exp(-dist/0.3) * diff / dist[:, np.newaxis]
            forces[i] -= repulsive_force.sum(axis=0)

        # Repulsive force from obstacles
        obstacle_diff = positions[:, np.newaxis, :] - obstacles[np.newaxis, :, :]
        obstacle_dist = np.linalg.norm(obstacle_diff, axis=2)
        obstacle_force = np.exp(-obstacle_dist/0.5)[:, :, np.newaxis] * obstacle_diff
    ...
    def calculate_congestion(self, positions):
        ...
        close_pedestrians = (distances < 0.5).sum() - self.num_pedestrians
        return close_pedestrians / self.num_pedestrians

    def all_pedestrians_exited(self, positions, exit_positions):
        distances_to_exits = np.linalg.norm(positions[:, np.newaxis] - exit_positions, axis=2)
        return np.all(np.min(distances_to_exits, axis=1) < 0.5)
    ...
    def calculate_evacuation_efficiency(self, avg_evacuation_time):
        ...
        return ideal_time / avg_evacuation_time  # Higher value indicates better efficiency
...
if __name__ == "__main__":
    ...
    print("Pedestrian Flow:", pedestrian_flow.simulate())